---
title: Contradiction Taxonomy - Formal Classification of Inconsistencies
description: Eight-type classification system for detecting contradictions in institutional documents, from internal inconsistencies to unexplained position reversals.
...
Classical logical contradiction is narrow: Proposition *p* and its negation not-*p* cannot both be true. But institutional documents exhibit a much wider range of problematic patterns:
...
Document states *p* and also states not-*p*.
...
Document states *p* and *q* where *p* logically entails not-*q*.
...
- **Quotations vs. assertions**: If document quotes Person A saying *p* and Person B saying not-*p*, this is not SELF (it's reporting disagreement). But if document *asserts* both *p* and not-*p*, it's SELF.
...
- Document *D1* asserts proposition *p*
- Document *D2* asserts proposition *q*
...
- Event *E1* stated to occur before *E2*, but *E2* stated to occur before *E1*, or
- Document dated time *t1* describes events at time *t2* where *t2* > *t1* (future events)
...
Document 1 states *A* then *B* then *C*; Document 2 states *B* then *C* then *A*.
...
- Document D1 presents claim with modal qualifier *M1* (e.g., "possibly," "may have")
- Document D2 presents same claim with different modal qualifier *M2* (e.g., "definitely," "did")
...
- Removal of "not" changing meaning entirely (abuse uncertain to abuse confirmed)

**High**:
- Substantial certainty increase (possible to definite) affecting important conclusions
- Multiple modality shifts compounding

**Medium**:
- Modest certainty increase (possible to probable)
...
- Document D1 makes claim *C1* with scope *S1* (temporal, quantitative, or qualificational)
- Document D2 makes claim *C2* with scope *S2*
- *C1* and *C2* are about same subject
- *S2* > *S1* (broader scope) or *S2* < *S1* (narrower scope)
...
- Contextual qualifiers removed changing meaning (defensive response to aggressive behavior)
...
**Medium**:
- Modest scope broadening (one to two instances)
...
- Institution or document author takes position *P1* at time *t1*
- Same institution/author takes position *P2* at time *t2*
- *P2* contradicts or substantially diverges from *P1*
