...
function checkClaudeCodeStatus() {
  try {
    const version = (0, import_child_process.execSync)("claude --version", {
      encoding: "utf-8",
      timeout: 5e3,
      stdio: ["pipe", "pipe", "pipe"],
      env: { ...process.env, ANTHROPIC_API_KEY: void 0 }
      // Ensure no API key
    }).trim();
    return {
      installed: true,
      authenticated: true,
      // Assumed - will fail on first use if not
      version
    };
  } catch (error) {
...
${userContent}`;
    const child = (0, import_child_process.spawn)("claude", ["--print", "-p", "-"], {
      stdio: ["pipe", "pipe", "pipe"],
      env: {
        ...process.env,
        ANTHROPIC_API_KEY: void 0,
        // Force Max subscription usage
        NO_COLOR: "1"
    ...
    timeoutId = setTimeout(() => {
      if (!resolved) {
        ...
        safeResolve({
          success: false,
          content: "",
          error: `Claude Code timed out after ${timeout / 1e3} seconds`
        });
      }
    }, timeout);
  });
...
function loadConfig() {
  const configPaths = [
    path2.join(process.env.APPDATA || "", "com.apatheia.phronesis", "config.json"),
    path2.join(process.env.HOME || "", ".phronesis", "config.json"),
    path2.join(__dirname, "config.json")
  ];
...
function getDatabase(config) {
  if (!readDb) {
    ...
    readDb = new import_better_sqlite3.default(config.database_path, { readonly: true });
    log(`Connected to database (read): ${config.database_path}`);
  }
  return readDb;
}
function getWriteDatabase(config) {
  if (!writeDb) {
    if (!fs.existsSync(config.database_path)) {
      throw new Error(`Database not found at ${config.database_path}`);
    }
    writeDb = new import_better_sqlite3.default(config.database_path);
...
function loadDocuments(config, documentIds) {
  try {
...
function loadDocumentContent(config, documentId) {
  try {
    const database = getDatabase(config);
    ...
    `);
    const chunks = chunksStmt.all(documentId);
...
function saveFindings(config, caseId, findings) {
  try {
...
async function callWithRetry(fn, maxRetries = 3, baseDelayMs = 1e3, maxDelayMs = 1e4) {
  let lastError = null;
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (errorMessage.includes("invalid api key") || errorMessage.includes("authentication") || errorMessage.includes("not configured")) {
        throw lastError;
      }
      const isRetryable = errorMessage.includes("rate limit") || errorMessage.includes("timeout") || errorMessage.includes("network") || errorMessage.includes("overloaded") || errorMessage.includes("529") || errorMessage.includes("500") || errorMessage.includes("502") || errorMessage.includes("503") || errorMessage.includes("504");
      if (!isRetryable) {
        throw lastError;
      }
...
async function generateJSON(config, systemPrompt, userContent) {
  if (config.use_claude_code) {
    log("Using Claude Code for AI request");
    const result = await executeJSONPrompt(systemPrompt, userContent, { timeout: 18e4 });
    if (!result.success) {
...
var contradictionEngine = async (config, caseId, documentIds) => {
  ...
  const formattedDocs = docContents.map(
...
Respond in JSON:
{
  ...
  const result = await generateJSON(config, "You are a forensic document analyst.", prompt);
...
${sanitizeForPrompt(sourceContent, 4e4)}

PROFESSIONAL REPORT (${sanitizeFilename(reportDoc?.filename || "Unknown")}):
${sanitizeForPrompt(reportContent, 4e4)}
...
var expertWitnessEngine = async (config, caseId, documentIds) => {
  const docs = loadDocuments(config, documentIds);
...
EXPERT REPORT:
${sanitizeForPrompt(reportContent, 6e4)}
...
var narrativeEngine = async (config, caseId, documentIds) => {
  const docs = loadDocuments(config, documentIds);
  if (docs.length < 2) {
    return [];
  }
  ...
  const docContents = sortedDocs.map((doc) => ({
    ...
    name: sanitizeFilename(doc.filename),
    date: doc.created_at,
    content: sanitizeForPrompt(loadDocumentContent(config, doc.id), 3e4)
  }));
...
Respond in JSON:
{
  ...
  const result = await generateJSON(config, "You are a forensic narrative analyst.", prompt);
  return (result.evolutions || []).filter((e) => e.drift_type !== "stable").map((e, idx) => ({
    id: v4_default(),
    engine_id: "narrative",
...
var coordinationEngine = async (config, caseId, documentIds) => {
  const docs = loadDocuments(config, documentIds);
  if (docs.length < 2) {
    return [];
  }
...
var entityResolutionEngine = async (config, caseId, documentIds) => {
  const docs = loadDocuments(config, documentIds);
  const allContent = docs.map((doc) => sanitizeForPrompt(loadDocumentContent(config, doc.id), 4e4)).join("\n\n~~~\n\n");
  const prompt = `Extract all named entities and resolve aliases/references to canonical identities.

DOCUMENTS:
${allContent.slice(0, 8e4)}
...
Respond in JSON:
{
  ...
  `);
  const insertEntities = database.transaction((entities) => {
...
var temporalParserEngine = async (config, caseId, documentIds) => {
  ...
  const docContents = docs.map((doc) => ({
    id: doc.id,
    name: sanitizeFilename(doc.filename),
    content: sanitizeForPrompt(loadDocumentContent(config, doc.id), 4e4)
  }));
...
Respond in JSON:
{
  ...
  const result = await generateJSON(config, "You are a timeline analyst.", prompt);
  const database = getWriteDatabase(config);
  ...
  `);
  const insertEvents = database.transaction((events) => {
  ...
  const findings = (result.anomalies || []).map((a, idx) => ({
    id: v4_default(),
    engine_id: "temporal",
    finding_type: a.type,
...
var documentaryEngine = async (config, caseId, documentIds) => {
  const findings = [];
  const docs = loadDocuments(config, documentIds);
  for (const doc of docs) {
    const hasEditorialIndicators = doc.filename.toLowerCase().includes("draft") || doc.filename.toLowerCase().includes("version") || doc.extracted_text && doc.extracted_text.includes("redacted");
    if (hasEditorialIndicators) {
      findings.push({
        ...
        document_ids: [doc.id],
        evidence: {
          indicators: hasEditorialIndicators ? ["filename_suggests_draft", "content_redaction"] : []
        },
        metadata: {}
      });
    }
...
var argumentationEngine = async (config, caseId, documentIds) => {
  const findings = [];
  const docs = loadDocuments(config, documentIds);
  for (const doc of docs) {
...
var biasDetectionEngine = async (config, caseId, documentIds) => {
  const findings = [];
  const docs = loadDocuments(config, documentIds);
  for (const doc of docs) {
    if (!doc.extracted_text) continue;
...
var accountabilityAuditEngine = async (config, caseId, documentIds) => {
  const findings = [];
  const docs = loadDocuments(config, documentIds);
  for (const doc of docs) {
    if (!doc.extracted_text) continue;
...
var promptExecutorEngine = async (config, caseId, documentIds, options) => {
  const systemPrompt = options?.system_prompt;
  const userContent = options?.user_content;
