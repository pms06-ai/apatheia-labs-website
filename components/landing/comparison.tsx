import { enterpriseTools, phronesisFeatures } from '@/lib/content';
import { Info } from 'lucide-react';

export function Comparison() {
  return (
    <section className="py-20 md:py-28">
      <div className="mx-auto max-w-[var(--container-content)] px-6">
        <div className="mx-auto max-w-2xl text-center">
          <p className="text-sm font-medium uppercase tracking-wider text-bronze-500">
            The Playing Field
          </p>
          <h2 className="mt-3 font-serif text-3xl md:text-4xl">
            What You&rsquo;re Up Against
          </h2>
          <p className="mt-4 text-charcoal-400">
            These are the tools institutions use to build cases against people
            like you.
          </p>
        </div>

        <div className="mt-14 grid gap-6 md:grid-cols-2">
          {/* Enterprise */}
          <div className="rounded-xl border border-charcoal-800 bg-charcoal-850 p-6">
            <div className="flex items-baseline justify-between">
              <h3 className="text-lg font-medium text-charcoal-100">
                What They Have
              </h3>
              <span className="text-xs font-medium uppercase tracking-wider text-charcoal-500">
                Enterprise Tools
              </span>
            </div>
            <ul className="mt-6 divide-y divide-charcoal-800">
              {enterpriseTools.map((tool) => (
                <li
                  key={tool.name}
                  className="flex items-center justify-between py-3 text-sm"
                >
                  <span className="text-charcoal-300">{tool.name}</span>
                  <span className="font-medium text-charcoal-400">
                    {tool.price}
                  </span>
                </li>
              ))}
            </ul>
            <p className="mt-4 text-xs text-charcoal-500">
              Plus teams of analysts trained to use them.
            </p>
          </div>

          {/* Phronesis */}
          <div className="rounded-xl border border-bronze-600/30 bg-charcoal-850 p-6">
            <div className="flex items-baseline justify-between">
              <h3 className="text-lg font-medium text-charcoal-100">
                What You Have Now
              </h3>
              <span className="text-xs font-medium uppercase tracking-wider text-bronze-500">
                Phronesis
              </span>
            </div>
            <ul className="mt-6 divide-y divide-charcoal-800">
              {phronesisFeatures.map((f) => (
                <li
                  key={f.name}
                  className="flex items-center justify-between py-3 text-sm"
                >
                  <span className="text-charcoal-200">{f.name}</span>
                  <span className="font-semibold text-bronze-500">
                    {f.price}
                  </span>
                </li>
              ))}
            </ul>
            <p className="mt-4 text-xs text-charcoal-400">
              And it runs on your computer. No subscriptions. No cloud.
            </p>
            <p className="mt-2 flex items-start gap-1.5 text-xs text-charcoal-500">
              <Info size={14} className="mt-0.5 shrink-0" />
              AI features use your own API keys. Typical usage: $5-20/month.
            </p>
          </div>
        </div>
      </div>
    </section>
  );
}
